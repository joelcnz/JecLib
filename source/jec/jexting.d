...
+/
	...
	void chunkCate(dstring str, int chunkSize) {
		...
		foreach(i, txt; txts) {
			...
			_txts[$ - 1].position = _txts[$ - 1].position + Vector2f(0, i * _textHeight );
		}
	...
	void position(float x, float y) {
		foreach(i, txt; _txts) {
			txt.position = Vector2f(x, y + i * _textHeight);
		}
	}
	...
	void draw() {
		if (edge) {
			...
			foreach(etxt; _txts) {
				...
				foreach(y; 0 .. 3)
					foreach(x; 0 .. 3) {
						edgeTxt.position = Vector2f(posx + x, posy + y);
